...
export async function POST(request: NextRequest) {
  try {
    ...
    await admin.database.from("integrations").update({ connected: false, state: null, updated_at: new Date().toISOString() }).eq("user_id", userId).eq("platform", "notion");

    return NextResponse.json({ ok: true, platform: "notion", userId });
  } catch (err) {
